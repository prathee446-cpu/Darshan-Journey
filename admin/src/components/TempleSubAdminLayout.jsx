...
import { 
  LayoutDashboard, ShieldCheck, Library, BookOpen, User, 
  LogOut, Menu, X, Bell, Globe, Calendar, Sparkles 
} from 'lucide-react';
...
export default function TempleSubAdminLayout({ children }) {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/sub-admin/temple/dashboard', icon: LayoutDashboard },
    { name: 'Temple Overview', path: '/sub-admin/temple/overview', icon: ShieldCheck },
    { name: 'Temple Services', path: '/sub-admin/temple/services', icon: Library },
    { name: 'Temple Bookings', path: '/sub-admin/temple/bookings', icon: BookOpen },
    { name: 'Profile', path: '/sub-admin/temple/profile', icon: User },
  ];

  const isSelected = (path) => {
    if (path === '/sub-admin/temple/dashboard') {
      return location.pathname === '/sub-admin/temple/dashboard' || location.pathname === '/sub-admin/temple';
    }
  ...
  const getPageTitle = () => {
    ...
    return active ? active.name : 'Temple In-Charge Sanctum';
  };
        ...
        <div style={{ padding: '0.85rem 1rem 0.2rem' }}>
          ...
          >
            ...
            <div style={{ fontSize: '0.85rem', fontWeight: '700', color: '#FFFDF9', marginTop: '2px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
              🛕 {currentUser.temple || 'Kapaleeshwarar Temple'}
            </div>
            <div style={{ fontSize: '0.7rem', color: 'var(--admin-text-muted)', marginTop: '2px' }}>
              City: {currentUser.branch || 'Chennai'}
            </div>
          </div>
        </div>
              ...
              <div style={{ padding: '1.25rem', borderBottom: '1px solid rgba(214, 181, 109, 0.12)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                  ...
                  <div>
                    ...
                    <span style={{ fontSize: '0.62rem', color: '#8EAE68', display: 'block' }}>Temple In-Charge</span>
                  </div>
                </div>
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem' }}>
            ...
            >
              <ShieldCheck size={13} />
              <span>{currentUser.temple || 'Kapaleeshwarar Temple'} · {currentUser.branch || 'Chennai'}</span>
            </div>
