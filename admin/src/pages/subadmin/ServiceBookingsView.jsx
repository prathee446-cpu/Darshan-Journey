...
import { BookOpen, Search, Filter, CheckCircle2, Clock, X, Check, RefreshCw, Sparkles, User, Calendar, Phone } from 'lucide-react';
import { getAuthHeaders, getCurrentUser } from '../../utils/auth';

export default function ServiceBookingsView() {
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 className="serif-title" style={{ fontSize: '1.6rem', color: '#FFFDF9', margin: '0 0 0.25rem' }}>
            Bookings Ledger — {serviceInfo.name || 'Pooja Service'}
          </h2>
          <p style={{ color: 'var(--admin-gold)', fontSize: '0.85rem', margin: 0 }}>
            Filtered exclusively for your assigned service reservations
          </p>
        </div>
        ...
        >
          <RefreshCw size={14} className={loading ? 'spin-slow' : ''} /> Refresh Ledger
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '240px', position: 'relative' }}>
          ...
          <input
            type="text"
            placeholder="Search by devotee, ID, phone, subcategory..."
            value={searchTerm}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              <thead>
                <tr style={{ backgroundColor: 'rgba(18, 9, 7, 0.4)', borderBottom: '1px solid rgba(214, 181, 109, 0.15)', color: 'var(--admin-gold)', fontSize: '0.75rem', textTransform: 'uppercase' }}>
                  ...
                  <th style={{ padding: '0.85rem 1rem' }}>Devotee Name</th>
                  <th style={{ padding: '0.85rem 1rem' }}>Contact</th>
                  <th style={{ padding: '0.85rem 1rem' }}>Subcategory</th>
                  <th style={{ padding: '0.85rem 1rem' }}>Ritual Date</th>
