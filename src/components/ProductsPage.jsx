...
export default function ProductsPage({ onGoToHome, onGoToLanding, onExploreTemples }) {
  ...
  const handleNext = () => {
    setDirection(1);
    setCurrentIndex((prev) => (prev + 1) % PRODUCT_SLIDES.length);
  };

  const handlePrev = () => {
    setDirection(-1);
    setCurrentIndex((prev) => (prev - 1 + PRODUCT_SLIDES.length) % PRODUCT_SLIDES.length);
  };
  ...
  return (
    <div 
      className="home-website-wrapper products-page-container" 
      style={{
